...
+-----------------+
  ...
  ** Clone repository at Gitbash **
    ...
    *https://us-east-2.console.aws.amazon.com/codesuite/codecommit/repositories/MLP197/browse?region=us-east-2#
    * Choose Clone URL, and then copy the SSH URL.    
    * Go back to Gitbash and clone the project
    * `pwd` -- Check and verify that the current working directory is C:\users\Hvuser/workspace
    * `git clone ssh://git-codecommit.us-east-2.amazonaws.com/v1/repos/MLP197 `
    * `cd MLP197`
    * `git status`
  * open Visual Studio (VS) Code
    * Open folder c:\users\Hvuser\workspace\MLP197
...
# Workshop material - Day #2 - MySQL Workbench/Cli/Curl/Browser to interact w/ application
  ...
  * `CREATE DATABASE MLP197;` and click the lightning button
  * `CREATE USER 'MLP197'@'localhost' IDENTIFIED BY 'MLP197';`
  * `GRANT ALL ON MLP197.* TO 'MLP197'@'localhost';`
  ...
  * go to gitbash, ensure you are in workspace/MLP197
  * `cd restservice/leavemanager`
  * `mvn compile`
  * `mvn exec:java -Dexec.mainClass=com.hexaware.MLP197.util.CliMain`
  ...
  * Build the war file as follows:
    ...
    * `cp target/MLP197-0.0.1-SNAPSHOT.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/MLP197.war`
    * start tomcat and tail its logs
      * `cd D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16`
      * `rm -rf logs/*`
      * `./bin/startup.sh`
      * `tail -f logs/*`
      * `curl -vvv http://localhost:8080/MLP197/api/employees | python -m json.tool`
      * `curl -vvv http://localhost:8080/MLP197/api/employees/2000 | python -m json.tool`
   * `cd ../../webui/lm-app/`
   * `npm install`
   * `ng build`
   * `cp ../../restservice/leavemanager/target/MLP197-0.0.1-SNAPSHOT.war ./MLP197.war`
   * `cd dist`
   * `jar -uvf ../MLP197.war *`
   * `cd ..`
   * `jar -tvf MLP197.war`
   * `cp MLP197.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/MLP197.war`
   * Notice that the tail terminal shows that the new version of the web application archive is now getting deployed
   * Open Chrome browser and navigate to http://localhost:8080/MLP197/
...
# Workshop material - Day #4 - Configure Project Board in Trello for  Managing and Tracking Your Project
  ...
  * Create a board for your project as "MLP197"
  ...
  * [Team member #3]
     * Create "Current Sprint" with "MLP197 Sprint 1" as label 
